...
const CHECKOUT_ID = "checkout_123";
...
function duplicateEventError(): Prisma.PrismaClientKnownRequestError {
  return new Prisma.PrismaClientKnownRequestError("Duplicate Chargily event", {
    code: "P2002",
    clientVersion: "test",
  });
}
...
function toPrismaClientStub(stub: PrismaStub): PrismaClient {
  if (typeof stub.$transaction !== "function") {
...
function signedPayload(plan: Plan): { raw: string; signature: string } {
  ...
  return {
    raw,
    signature: crypto.createHmac("sha256", CHARGILY_SECRET).update(raw).digest("hex"),
  };
}

async function buildApp(state: PaymentState): Promise<ReturnType<typeof Fastify>> {
  const routeModule = await import("./chargily.js");
  const routeCandidate: unknown = routeModule.default;
  if (typeof routeCandidate !== "function") {
    throw new Error("Invalid Chargily webhook route plugin");
  }
...
describe("Chargily webhook", () => {
  ...
  it("processes signed checkout.paid events once", async () => {
    const state: PaymentState = {
      eventIds: new Set(),
      paymentEventsCreated: 0,
      subscriptionPlan: Plan.FREE,
    ...
    const app = await buildApp(state);
    const { raw, signature } = signedPayload(Plan.PRO);
    ...
    assert.equal(state.subscriptionPlan, Plan.PRO);
    assert.equal(state.tenantPlan, Plan.PRO);
